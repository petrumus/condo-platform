...
| 2026-03-01 | F15 email notifications fully activated: edge function deployed, `N8N_WEBHOOK_BASE_URL` + `N8N_WEBHOOK_SECRET` secrets set, fire-and-forget webhook calls wired into announcements/ballots/initiatives/maintenance server actions. All email workflows live. |
...
| 2026-03-01 | Hotfix on main (F15): `triggerN8nWebhook` was routing through a Supabase Edge Function that was never deployed — all invitation/email webhooks silently dropped. Fixed by calling n8n directly from server actions using `N8N_WEBHOOK_BASE_URL` + `N8N_WEBHOOK_SECRET` env vars. Errors now logged instead of swallowed. Vars added to `.env.local` and Vercel. Also fixed: `void` → `await` on all 8 webhook calls (Vercel kills unawaited fetches), and trailing-slash sanitisation on `N8N_WEBHOOK_BASE_URL` (was producing `//webhook/` 404s). Email notifications now confirmed working end-to-end. |
| 2026-03-01 | Hotfix: RLS infinite recursion on `condominium_members` (PostgreSQL error 42P17). Root cause: policies on `condominium_members` used inline `EXISTS(SELECT 1 FROM condominium_members ...)` subqueries — PostgreSQL applies RLS to the inner query, re-triggering the same policies. Fix: created `is_member()` SECURITY DEFINER function (migration `20260301000000_fix_rls_recursion.sql`) and replaced 6 self-referencing policies on `condominium_members`, `condominiums`, and `functional_titles`. This blocked all RLS-enabled queries: tenant layout returned 404, middleware redirected to `/pending`, super-admin detail page showed 0 members. |
